...
<?php
    ...
    if(!$con){
        ...
        $sqlresident = "SELECT COUNT(*) AS SUM FROM households WHERE electricityid='$purokid'";
    ...
    $sqlmale = "SELECT COUNT(*) AS SUMMALE FROM households where electricityid='2'";
    $resultmale = mysqli_query($con,$sqlmale);
    $rowmale = mysqli_fetch_array($resultmale);

    $sqlfemale = "SELECT COUNT(*) AS SUMFEMALE FROM households where electricityid='3'";
...
<?php include 'navigation.php' ?>
    ...
    <div class="container">
        <div class="navbar">
            <img src="css/images/SystemLogo.png" class="logo"> <h3>|With Electricicty</h3>
                <navList>
                    <ulist id="menuList">
                        <list><a href=""></a></list>
                        <list><a href=""> </a></list>
        ...
        <div class="col-1">
                    ...
                    <br>
                    <h4>Total Number of Households <b>with Electricity:</b></h4>
                    <h2><?php echo $rowmale['SUMMALE']?></h2>

                    <br>
                    <h4>Total Number of Households <b> without Electricity:</b></h4>
                    ...
                    $error = NULL;
                        ...
                        $resultresident = $mysqli->query("SELECT * FROM households");
                            while($rowresident = mysqli_fetch_assoc($resultresident)) {
                                $electmean = $rowresident['electricityid'];
                    ?>
                            <?php
                               $resultservices = $mysqli->query("SELECT * FROM services where id='$electmean'");
