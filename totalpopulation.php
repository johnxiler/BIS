...
<?php
    ...
    if(!$con){
        ...
        $sql = "select * from purok order by id";
        $result = mysqli_query($con,$sql);
        while($row = mysqli_fetch_array($result)){
            $purok[] = $row['purokno'];
            $purokid = $row['id'];

        $sqlresident = "SELECT COUNT(*) AS SUM FROM residents WHERE purok='$purokid'";
...
<?php include 'navigation.php' ?>
    ...
    <div class="container">
        <div class="navbar">
            <img src="css/images/SystemLogo.png" class="logo"> <h3>| Analytics Total Population</h3>
                <navList>
                    <ulist id="menuList">
                        <list><a href=""></a></list>
                        <list><a href=""> </a></list>
