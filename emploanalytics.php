...
<head>
    ...
    <title>Employment Analytics</title>
...
<?php
    ...
    if(!$con){
        ...
        $sql = "select * from employment order by id";
        $result = mysqli_query($con,$sql);
        while($row = mysqli_fetch_array($result)){
            $purok[] = $row['employment'];
            $purokid = $row['employment'];

        $mysqli = NEW MySQLi('localhost','root','','bis');
        $sqlresident = "SELECT COUNT(*) AS SUM FROM residents WHERE employstat='$purokid'";
    ...
    $sqlfemale = "SELECT COUNT(*) AS SUMFEMALE FROM residents where employstat='Unemployed'";
    $resultfemale = mysqli_query($con,$sqlfemale);
    $rowfemale = mysqli_fetch_array($resultfemale);

    $sqlflash = "SELECT COUNT(*) AS SUMflash FROM residents where employstat='Self-employed'";
    $resultflash = mysqli_query($con,$sqlflash );
    $rowflash = mysqli_fetch_array($resultflash );

    $sqlnone = "SELECT COUNT(*) AS SUMnone FROM residents where employstat='None'";
...
<?php include 'navigation.php' ?>
    ...
    <div class="container">
        <div class="navbar">
            <img src="css/images/SystemLogo.png" class="logo"> <h3>| Analytics Employment</h3>
                <navList>
                    <ulist id="menuList">
                        <list><a href=""></a></list>
                        <list><a href=""> </a></list>
        ...
        <div class="col-1">
                    ...
                    <br>
                    <h4>Total Number of Residents that are <b>Employed</b></h4>
                    <h2><?php echo $rowmale['SUMMALE']?></h2>

                    <br>
                    <h4>Total Number of Residents that are <b>Unemployed</b></h4>
                    <h2><?php echo $rowfemale['SUMFEMALE']?></h2>

                    <br>
                    <h4>Total Number of Residents that are <b>Self-Employed</b></h4>
                    <h2><?php echo $rowflash['SUMflash']?></h2>

                    <br>
                    <h4>Total Number of Residents that are <b>None Work</b></h4>
                    ...
                    $error = NULL;
                        ...
                        $resultresident = $mysqli->query("SELECT * FROM residents");
                            while($rowresident = mysqli_fetch_assoc($resultresident)) {
                                $electmean = $rowresident['employstat'];
                    ?>
                            <?php
                               $resultservices = $mysqli->query("SELECT * FROM employment where employment='$electmean'");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo $rowservices['employment']; ?></td>
